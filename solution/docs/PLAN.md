...
### Phase 6 — Monitoring, Drift Detection & Governance  *(BUILT — see `phase6_monitoring/`)*

**Goal:** long-term reliability and compliance.

**Work**
- Data validation gate on incoming requests (range/enum/schema) reusing the
  Phase 2 rule registry (`capstone.data_quality`), run as a batch gate over
  served request payloads.
- Drift monitoring off `prediction_log`: feature drift (PSI / KS), prediction-
  distribution drift (class-mix PSI), and — once outcomes are joined back on the
  caller's `visit_id` — performance drift (recall on the costly class + net
  recovered vs the Phase 4 baseline). The reference is the Phase 3 **test**
  window (the last validated distribution; the training window's calendar
  features are disjoint from any later window by construction).
- Scheduled drift job (`drift_job.py`) + a `scheduler.py` sidecar; threshold
  alerts from one source (`capstone.monitoring.ALERT_RULES`); `drift_report`
  table (one append-only row per run/metric).
- Audit log: `v_prediction_audit` (every prediction + versions) +
  `prediction_override` (who/what/when for manual overrides); append-only
  **enforced** by a trigger (`UPDATE` blocked on all three logs, `DELETE` also
  on the audit tables).
- Grafana dashboard (`grafana/hospital_drift.json`) over `drift_report` /
  `prediction_log`, provisioned in `docker compose` alongside Postgres, the API
  and the scheduler.
- Governance docs: `governance.md` (assumptions, limitations, monitoring design,
  roles, data handling), `retraining_policy.md` (triggers + procedure +
  rollback + sign-off), `runbook.md` (incident response).
- Reusable logic in `src/capstone/monitoring.py`. Deliverable is the notebook
  `phase6.ipynb` (per "notebooks from Phase 2 on"), regenerated by
  `run_phase6.py`, which seeds a baseline + a deliberately-drifted traffic
  window to exercise the monitor end-to-end.

**Deliverables:** `phase6_monitoring/` — `notebook.py` → `phase6.ipynb`,
`run_phase6.py`, `seed_traffic.py`, `drift_job.py`, `scheduler.py`,
`sql/drift_report.sql`, `grafana/`, `Dockerfile`, `docker-compose.yml`,
`governance.md`, `retraining_policy.md`, `runbook.md`, `tests/`,
`PHASE6_FINDINGS.md`.

**Exit criteria:** drift job runs on a schedule and alerts on injected drift ✓
(baseline window OK, drifted window ALERT — feature PSI on billed amount /
department / insurer / age, prediction-mix PSI, and a ~34-point Model B recall
drop); audit log immutable-by-convention ✓ (trigger-enforced); governance docs
reviewed ✓.
